...
router.put('/messages/read', auth, async (req, res) => {
  try {
    ...
    if (message_ids && Array.isArray(message_ids)) {
      ...
      const adminIds = (await db.collection('users').find({ role: 'admin' }).project({ _id: 1 }).toArray()).map(a => a._id);
      await db.collection('messages').updateMany(
        { receiver_id: userId, sender_id: { $in: adminIds }, is_read: false },
        { $set: { is_read: true } }
      );
    }
...
router.get('/conversation/:clientId', adminAuth, async (req, res) => {
  try {
    ...
    const messages = await db.collection('messages')
      .find({
        $or: [
          { sender_id: new ObjectId(clientId), receiver_id: null },
          { receiver_id: new ObjectId(clientId), sender_id: { $in: (await db.collection('users').find({ role: 'admin' }).project({ _id: 1 }).toArray()).map(admin => admin._id) } }
        ]
      })
      .sort({ created_at: -1 })
      .skip(Number(skip))
    ...
    // Mark messages as read
    await db.collection('messages').updateMany(
      { sender_id: new ObjectId(clientId), receiver_id: null, is_read: false },
      { $set: { is_read: true } }
    );
    const adminIds2 = (await db.collection('users').find({ role: 'admin' }).project({ _id: 1 }).toArray()).map(a => a._id);
    await db.collection('messages').updateMany(
      { receiver_id: new ObjectId(clientId), sender_id: { $in: adminIds2 }, is_read: false },
      { $set: { is_read: true } }
    );
