...
router.get('/clients/:id', adminAuth, async (req, res) => {
  try {
    ...
    const { id } = req.params;
    // Get client info
    const client = await db.collection('users').findOne({ _id: new ObjectId(id), role: 'client' });
    if (!client) {
      return res.status(404).json({ success: false, message: 'Client not found' });
    }
    // Get investments with custom amounts
    const investments = await db.collection('investments').find({ user_id: new ObjectId(id) }).sort({ created_at: -1 }).toArray();
    // Get withdrawals
    const withdrawals = await db.collection('withdrawals').find({ user_id: new ObjectId(id) }).sort({ created_at: -1 }).toArray();
    res.json({
      success: true,
...
router.put('/issues/:id', adminAuth, async (req, res) => {
  try {
    ...
    const issue = await db.collection('password_reset_requests').findOne({ _id: new ObjectId(id) });
    if (!issue) {
      return res.status(404).json({
        success: false,
...
router.get('/analytics', adminAuth, async (req, res) => {
  try {
    ...
    const totalEarnedAgg = await db.collection('client_balances').aggregate([{ $group: { _id: null, total: { $sum: '$balance' } } }]).toArray();
...
router.post('/credit', adminAuth, async (req, res) => {
  try {
    ...
    const user = await db.collection('users').findOne({ _id: new ObjectId(user_id), role: 'client' });
    if (!user) {
      return res.status(404).json({ success: false, message: 'Client not found' });
    }
    // Try to update balance
    const result = await db.collection('client_balances').updateOne({ user_id: new ObjectId(user_id) }, { $inc: { balance: amount } }, { upsert: true });
    const updated = await db.collection('client_balances').findOne({ user_id: new ObjectId(user_id) });
...
router.get('/client-customers', adminAuth, async (req, res) => {
  try {
    ...
    let query = { client_id: client_id ? new ObjectId(client_id) : { $exists: true } };
    ...
    const clientsSummary = await db.collection('users').aggregate([
      ...
      },
      {
        $addFields: {
          ...
          total_amount: { $sum: { $map: { input: { $filter: { input: '$customers', as: 'c', cond: { $eq: ['$$c.status', 'completed'] } } }, as: 'x', in: '$$x.amount' } } },
