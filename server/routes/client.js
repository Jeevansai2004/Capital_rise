...
router.post('/invest', clientAuth, async (req, res) => {
  try {
    ...
    const db = await connectToDatabase();
    const loot = await db.collection('loots').findOne({ _id: typeof loot_id === 'string' ? new ObjectId(loot_id) : loot_id, is_active: true });
    if (!loot) {
    ...
    const result = await db.collection('investments').insertOne({
      user_id: userId,
      loot_id: typeof loot_id === 'string' ? new ObjectId(loot_id) : loot_id,
      amount: total_amount,
...
router.get('/referrals', clientAuth, async (req, res) => {
  try {
    ...
    const referrals = await db.collection('referrals').find({ investment_id: { $in: (await db.collection('investments').find({ user_id: userId }).project({ _id: 1 }).toArray()).map(x => x._id) } }).sort({ created_at: -1 }).toArray();
